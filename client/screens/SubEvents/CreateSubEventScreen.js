...
const CreateSubEvent = (params) => {
    ...
    const handleSubmit = async () => {
        ...
        const { name: subeventName, location: subeventLocation, date: subeventDate, startTime: subeventtartTime, endTime: subeventEndTime } = subEvent;
        ...
        <ScrollView style={styles.container}>
            <View style={styles.section}>
                ...
                <View style={styles.section}>
                    <TextInput
                        ...
                        placeholder="Name"
                        value={subEvent.name}
                        onChangeText={(value) => handleUpdateSubEvent('name', value)}
                        required={true}
                    />
                    <TextInput
                        style={styles.input}
                    ...
                    <TouchableOpacity style={styles.outlineButton}>
                        <TouchableOpacity onPress={() => setOpen(true)}>
                            <Text style={styles.outlineButtonText}>
                                {subEvent.date 
                                ? new Date (subEvent.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })
                                : 'Select Date'}
                        </Text>
                        </TouchableOpacity>
                        <SafeAreaProvider>
                            <DatePickerModal
                                ...
                                onConfirm={(selectedDate) => {
                                    handleUpdateSubEvent('date', selectedDate.date)
                                    setOpen(false);
                                }}
                                saveLabel="Save" 
                                required={true}
                    ...
                    <TouchableOpacity style={styles.outlineButton} required={true} onPress={() => { setVisible(true); setSelectedEventType('start'); }}>
                        <Text style={styles.outlineButtonText}>
                            {subEvent.startTime 
                                ? subEvent.startTime
                                : 'Select Start Time'
                    ...
                    <TouchableOpacity style={styles.outlineButton} required={true} onPress={() => { setVisible(true); setSelectedEventType('end'); }}>
                        <Text style={styles.outlineButtonText}>
                            {subEvent.endTime 
                                ? subEvent.endTime
                                : 'Select End Time'
            ...
            <View>
                <TouchableOpacity style={styles.submitButton} onPress={handleSubmit}>
                    <Text style={styles.submitButtonText}>Create Sub-Event</Text>
                </TouchableOpacity>
            </View>
        </ScrollView>
        </KeyboardAvoidingView>
